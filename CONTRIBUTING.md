...
- Run `yarn watch --force` to build `angular-gantt.js` on the fly during development.
...
- Update the demo application and run it by running `yarn serve` from `demo` folder.
- Update `README.md` if required.
- Build demo by running `yarn build-demo`.
...
1. `yarn grunt release-it[:<version>|patch|minor|major] [--no-write]`
    ...
    `yarn grunt dist` automates the preparation of release, and can be run manually to check if everything works as expected.
    ...
    `yarn grunt dist` won't commit or push anything. A dry-run of the release process can also be performed using `--no-write` option.
     
     Make sure both tasks are working properly before running `yarn grunt release-it` without the `--no-write` option.
